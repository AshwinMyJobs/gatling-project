 ...
 */
public class Scenario6_Login_InteropUserManagement extends Simulation {

    // 1. Endless loop data feeder targeting your core mapping configuration file
    private final FeederBuilder<String> userFeeder = csv("talk_group_management.csv").circular();

    // 2. HTTP Base Options locked to the precise environment subdomain routing properties
    ...
    private static final ChainBuilder interopUserManagement = group("Step 03: Interop User Management").on(
            exec(http("10. API: Get Master List Info")
                    ...
                    // Java 11 safe concatenated string payload layout matching your parameter bounds
                    .body(StringBody("{"
                            + "\"rowsPerPage\":\"200\","
                            + "\"pageNumber\":\"1\","
                            + "\"searchString\":\"\","
                            ...
                            + "\"offset\":\"0\","
                            + "\"userType\":\"lmrData\","
                            + "\"hierarchy_id\":\"#{hierarchy_id}\"," // 🌟 Dynamic hierarchy binding from CSV source
